...
% QR = [Q, R]
QR = [442, 368, 389, 332, 486, 139, 348, 4.9, 310, 6.2, 138, 135, 0.104, 0.639, 0.299, 9.86];
...
function RMSE_cost = LQ(A, B, QR, U_ref, x_ref)
    ...
    % Split cost array into Q and R costs
    Q = diag(QR(1:12));
    R = diag(QR(13:16));
...
% (x, y, z, phi, theta, psi)
lb = [zeros(1,12), 0.001*ones(1,4)];  % Q ≥ 0, R > 0
ub = [500*ones(1,12), 10*ones(1,4)];

% % GA setup for starting with random costs
% opts = optimoptions('ga', ...
%                     'InitialPopulationMatrix', rand(32, vars) .* (ub - lb) + lb, ...
%                     'MaxGenerations',1000, ... % Iterations
%                     'PopulationSize', 32, ... % Cost population size (2*number of vars)
%                     'Display','iter');

% GA setup for follow-on runs with predefined costs as a seed
randVars = rand(31, vars) .* (ub - lb) + lb; % Random costs
seedVars = [QR; randVars]; % Seed of chosen costs plus random costs

opts = optimoptions('ga', ...
                    'InitialPopulationMatrix', seedVars, ...
                    'MaxGenerations',250, ... % Iterations
                    'PopulationSize', 32, ... % Cost population size (2*number of vars)
                    'Display','iter');

% Define cost/fitness function
cost = @(QR) LQ(A, B, QR, U_ref, x_ref);
