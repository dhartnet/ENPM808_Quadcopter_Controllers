...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%% LQI Controller for Quadcopter Trajectory Tracking %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Wind speed noise is calculated using a exponential smoothing process to
...
QR = [500, 0.196, 378, 75.4, 500, 238, 169, 1.7, 94.6, 0.015, 125, 234, 500, 443, 0.002, 0.313, 0.143, 0.04, 9.4];

% Split out into Q (augmented for integral action states) and R costs
Q_mod = diag(QR(1:15));
R = diag(QR(16:19));
...
int_hist = zeros(n_int, N); % Stores integral error history
...
for i = 1:N-1
    ...
    % Update and store integral error
...
fprintf('Root Mean Squared Error for LQI Trajectory Tracker\n')
fprintf('- - - - - - - - - - - - - - -\n');
...
sgtitle('Quadcopter States with LQI Controller');
...
sgtitle('Quadcopter Control Inputs with LQI Controller')
...
grid on;
axis equal;
title('3D Position for Trajectory Tracking with LQI Controller');
...
xlabel('Time (s)');
ylabel('Integral Action Error (m)');
title('LQI Controller Integral Error States');
legend('X', 'Y', 'Z');
grid on;