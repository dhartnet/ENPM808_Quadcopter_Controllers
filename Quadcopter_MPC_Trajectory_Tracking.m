...
a_wind = F_wind ./ m; % Acceleration due to wind

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%% STATE SPACE MODEL & PLANT %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%% State Space model without Noise %%%%
...
%%%% State Space Model with Wind Modeled as Unmeasurable Disturbance %%%%
% To model unmeasured disturbance in the MPC Toolbox, need to model it with
% a "disturbance state observer" setup
% A = zeros(12); % Initialize A-matrix
% B = zeros(12, 4); % Initialize B-matrix
% C = [eye(12), zeros(12,2)]; % Assuming full observablity with 12 states (IMU and such assumed) and added wind (non observable)
% D = zeros(12,6); % Initialize D matrix with wind spots added
% 
% % Populate A-matrix
% A(1,2) = 1;
% A(2,9) = g;
...
% A(11, 12) = 1;
% 
% % Account for unmeasured wind disturbance
% A_wind = [zeros(12,1), zeros(12,1)];
% A_wind = [A, A_wind; zeros(2, 12), zeros(2,2)];
% 
% % Populate B-matrix
% B(6, 1) = 1/m;
% B(8, 2) = 1/Ixx;
...
% 
% % Account for unmeasured wind disturbance
% Bd = zeros(12,2); % Disturbance matrix
% Bd(2,1) = 1; % x-direction wind
% Bd(4,2) = 1; % y-direction wind
% B_wind = [B, Bd; zeros(2,6)]; % Total B-matrix
% 
% % Create state space form and plant from state matrices with wind
% sys = ss(A_wind, B_wind, C, D);
% plant = c2d(sys, dt);
% plant = minreal(plant); % Handle unobservable states (wind)
% 
% % Identify control inputs (MV) vs. unmeasurable wind (UD)
% plant = setmpcsignals(plant, 'MV', 1:4, 'UD', 5:6);
...
% Combined weights from GA optimization (plus post-GA manual tuning)
% weights = [MV, MVrate, OV]
weights = [0.192, 0.09, 0.16, 4.25, 0.1, 0.077, 0.2, 0.83, 42.15, 200, 147, 115, 200, 22, 0.5, 8.05, 0.2, 5.125, 149, 189];

% Separate weights
mpcobj.Weights.ManipulatedVariables = weights(1:4); % Penalize excessive deviations in inputs
mpcobj.Weights.ManipulatedVariablesRate = weights(5:8); % Penalize excessive rate of change in inputs
mpcobj.Weights.OutputVariables = weights(9:20); % Penalize deviations from desired states
...
% Initial control inputs for no noise
mpcobj.Model.Nominal.U = [m*g; 0; 0; 0];

% % Initial control inputs for added wind disturbance
% mpcobj.Model.Nominal.U = [m*g; 0; 0; 0; 0; 0];
...
for i = 1:N-1
    ...
    % e_z(i) = e(5);
    % 
    % % Calculate state change at time step with added wind disturbance
    % % Use non-wind state matrices and added disturbance matrix
    % dx = A * state(:,i) + B * U + Bd * a_wind([1,3],i);
    % dx(6) = dx(6) - g; % Account for gravity not represented in linearized state space form
...
sgtitle('Quadcopter States with MPC Controller');
...
sgtitle('Quadcopter Control Inputs with MPC Controller')
...
grid on;
axis equal;
title('3D Position for Trajectory Tracking with MPC Controller');